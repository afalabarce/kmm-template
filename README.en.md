...
On the rest of the platforms, we must define a file that will be the one in which we will implement the method (
or expect function) that we require:

- **[io.afalabarce.template.kmm.data.datasources.core.di.DataSourceCoreDependencyInjector.android.kt]**, the implementation of
  the class/functions using android's own elements.
- **[io.afalabarce.template.kmm.data.datasources.core.di.DataSourceCoreDependencyInjector.ios.kt]**, the implementation of the
...
As we can see, each implementation uses a getDatabaseBuilder method. This method is responsible for generating the logic for creating the Room database.
As we can see, platform-specific elements are used.

As we can see, once we mark the method **getPlatformInjects** as expect, all its implementations must go
marked as actual.
...
remote. Keep in mind that, since we use Room as a local persistence engine, we are going to have to define
our local entities in the same way as pure android projects:
...
- **Room**, such as "ORM" and data access layer.
...
Room allows us to greatly abstract [CRUD](https://es.wikipedia.org/wiki/CRUD) operations, the result
